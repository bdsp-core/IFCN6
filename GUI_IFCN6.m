function GUI_IFCN6
    ...
    %
    % Developed by Dr. Jin Jing (jjing@bidmc.harvard.edu)
    % 2023-June
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   
    %%
    warning('off','all');addpath('./Tools/');
    
    % Figure and axis settings
    f=figure('units','normalized','position',[0.0000 0.0333 1.0000 0.9475]);
    set(f,'MenuBar','none','ToolBar','none','HitTest','off','WindowButtonDownFcn',@clicks_Callback,'ButtonDownFcn',@clicks_Callback,'KeyPressFcn',@keys_Callback);

    Ax_EEG=subplot('position',[.05 .06 .70 .91],'Parent',f);
    Ax_spike=subplot('position',[.78 .60 .21 .37],'Parent',f);
    Ax_features={subplot('position',[.78 .06 .21 .05],'Parent',f);subplot('position',[.78 .15 .21 .05],'Parent',f);subplot('position',[.78 .24 .21 .05],'Parent',f);subplot('position',[.78 .33 .21 .05],'Parent',f);subplot('position',[.78 .42 .21 .05],'Parent',f);subplot('position',[.78 .51 .21 .05],'Parent',f)};
    startx=uicontrol(f,'style','pushbutton','units','normalized','position',[0.469 0.580 0.050 0.040],'string','Start','fontsize',15,'callback',@fcn_start);
    
    set(Ax_EEG,'Visible','off');set(Ax_spike,'Visible','off');for j=1:6;set(Ax_features{j},'Visible','off');end

    %% Initialization
    dataDir='./Data/';
    tmp=load('model_IFCN6');features=tmp.features;mdl=tmp.mdl;feature_mean=tmp.feature_mean;feature_std=tmp.feature_std;spike=tmp.spike;locs=tmp.locs;spike_centers=tmp.spike_centers;    
    data=[];timeStampo=[];ii=[];pp_x=[];pp_y=[];ff=[];file=[];    
    w=10;Fs=128;zScale=1/100;montage='Average';
    ...
    while true
        % parse data
        t_center=ii;t_o=t_center-(w/2*Fs);t_1=t_center+(w/2*Fs)-1;
        seg_=data(:,max(1,t_o):min(size(data,2),t_1));seg=NaN(size(data,1),w*Fs);
        ...
        eeg=seg(1:19,:);gap=NaN(1,size(eeg,2));
        
        switch montage
            case 'L-Bipolar'
                seg=fcn_bipolar(eeg);
                seg_disp=[seg(1:4,:);gap;seg(5:8,:);gap;seg(9:12,:);gap;seg(13:16,:);gap;seg(17:18,:)];channel_withspace=channel_withspace_bipolar;
            case 'Average' 
                seg=eeg-repmat(mean(eeg,1),size(eeg,1),1);
                seg_disp=[seg(1:8,:);gap;seg(9:11,:);gap;seg(12:19,:)];channel_withspace=channel_withspace_average;
            case 'Monopolar'
                seg=eeg;
                seg_disp=[seg(1:8,:);gap;seg(9:11,:);gap;seg(12:19,:)];channel_withspace=channel_withspace_monopolar;
        end
        seg_disp(seg_disp>300)=300;seg_disp(seg_disp<-300)=-300;       
        tto=t_o;tt1=t_1;tt=tto:tt1;M=size(seg_disp,1);
        ...
        hold(Ax_EEG,'on')
            for iSec=2:round((tt1-tto+1)/Fs)
                ta=tto+Fs*(iSec-1);line([ta ta],[0 M+1],'linestyle','--','color',[.5 .5 .5])
            end
            ...
            dt=tt1-tto+1;a=round(dt*4/5);xa1=tto+[a a+Fs-1];ya1=[3 3];xa2=tto+[a a];ya2=ya1+[0 100*zScale];
            text(xa1(1)-.7*a/10,mean(ya2),'100\muV','Color','b','FontSize',12);text(mean(xa1),2.5,'1 sec','Color','b','FontSize',12,'horizontalalignment','center');
            line(xa1,ya1,'LineWidth',2,'Color','b');line(xa2,ya2,'LineWidth',2,'Color','b');
        hold(Ax_EEG,'off')
        ...
        if length(pp_x)==5
            p5=pp_x;tmp_t1=p5(1)-Fs*1.5+1;tmp_t2=p5(1)+Fs*1.5;tmp_seg=eeg(:,tmp_t1:tmp_t2);% c2
            switch montage
            case 'L-Bipolar'
                seg=fcn_bipolar(tmp_seg);ch_disp=[1:4 NaN 5:8 NaN 9:12 NaN 13:16 NaN 17:18];
            case 'Average' 
                seg=tmp_seg-repmat(nanmean(tmp_seg,1),size(tmp_seg,1),1);ch_disp=[1:8 NaN 9:11 NaN 12:19];
            case 'Monopolar'
                seg=tmp_seg;ch_disp=[1:8 NaN 9:11 NaN 12:19];
            end
            pp5=p5-tmp_t1+1;
        ...
        for kk=1:6
            ...
            hold(Ax_features{k},'on');
                ...
                d=max(xm)-min(xm);ylim([min(xm)-d*.2 max(xm)+d*.2]);xlim([-.5 8.5]);
                set(Ax_features{k},'ytick',[],'yticklabel',[]);axis xy;
                if kk~=6
                ...
                if ~isempty(ff)
                    ff_k=ff(kk);yyk=(0:8);xxk=xm;xq=min(max(ff_k,min(xm)),max(xm));yq=interp1(xxk,yyk,xq);
        ...
        hold(Ax_spike,'on')       
            ...
            y3=-10;x3=(y3-b)/a;x4=x3+1;y4=a*x4+b;
            line(Ax_spike,[x3 x4 x4 x3],[y3 y4 y3 y3],'linewidth',1,'linestyle','-','color','r');

            dx=-0.3;dy=2;
            line(Ax_spike,[x4+dx x4+dx x4],[y3 y3-dy y3-dy],'linewidth',1,'linestyle','-','color','r');
            text(Ax_spike,x4+.3,(y3+y4)/2,'S_p_f');text(Ax_spike,(x3+x4)/2,y3+5,'1','HorizontalAlignment','center');
            
            set(Ax_spike,'fontsize',12,'xlim',[10 60]);axis off;
        hold(Ax_spike,'off')
    ...
    function fcn_start(varargin) 
        ...
        file=uigetfile([dataDir,'*.*'],'Select an edf file...');
        loading=uicontrol(f,'style','text','units','normalized','position',[0.469 0.580 0.052 0.05],'string','Loading...','FontSize',12);drawnow;
        if strcmpi(file(end-3:end), '.edf')
            % read edf 
            [hdr,data_org]=edfread([dataDir,file]);%data_org=-data_org;
            channels_org=strrep(strrep(hdr.label','EEG',''),'REF','');Fs_org=hdr.frequency(1);
            timeStampo=datetime([strrep(hdr.startdate,'.','/'),' ',strrep(hdr.starttime,'.',':')],'inputformat','dd/MM/yy HH:mm:ss');
            data=fcn_selecChannels(data_org,channels_org,Fs_org);
        elseif strcmpi(file(end-3:end), '.mat')
            % read mat 
        ...
        clear tmp;delete(loading);
        set(Ax_EEG,'Visible','on');set(Ax_spike,'Visible','on');for i=1:6;set(Ax_features{i},'Visible','on');end
        uiresume(f);
    end
    ...
    function keys_Callback(f,varargin)
        ...
        switch key
            case {'control'}
                iMontage=find(ismember(montages,montage)==1);iMontage=iMontage+1;
                if iMontage>3
                    iMontage=1;
                end
    ...
    function clicks_Callback(varargin)
        ...
        xy=get(gca,'CurrentPoint');kx=xy(1,1);ky=xy(1,2);
        switch click_type
            case {'normal'}  
                dd=(tt-kx).^2;
                [~,idx_x]=min(dd);
    ...
    function [spect,stimes,sfreqs]=mtspecgram(varargin)
        ...
        [sfreqs,findx]=getfgrid(Fs1,nfft,fpass);
        Nf=length(sfreqs);
        ...
        spect=squeeze(S);winmid=winstart+round(Nwin/2);stimes=winmid/Fs1;
    end
    ...
    function [tapers,pad,Fs,fpass,err,trialave,params]=getparams(params)
        ...
        tapers=params.tapers;pad=params.pad;Fs=params.Fs;fpass=params.fpass;err=params.err;trialave=params.trialave;
    end
    ...
    % Helper function that gets the frequency grid associated with a given fft based computation 
        ...
        df=Fs/nfft;f=0:df:Fs;f=f(1:nfft);
        if length(fpass)~=1
    ...
    function data=fcn_selecChannels(data_org,channels_org,Fs_org)
        % Select channels and resample
        channels_eeg={'Fp1';'F3';'C3';'P3';'F7';'T3';'T5';'O1';'Fz';'Cz';'Pz';'Fp2';'F4';'C4';'P4';'F8';'T4';'T6';'O2'};
        nCh=length(channels_eeg);idx_ch=NaN(nCh,1);
        for i=1:nCh
            jj=find(ismember(lower(channels_org),lower(channels_eeg{i})));
            if isempty(jj)
                disp(['missing EEG channel: ',channels_eeg{i}]);keyboard;
            else
                idx_ch(i)=jj;
            end
        end
        eeg_=data_org(idx_ch,:);

        idx_ch=find(cellfun(@isempty,regexpi(lower(channels_org),'ekg|ecg')));
        if isempty(idx_ch)
            ekg=zeros(1,size(eeg,2));
        else
            if length(idx_ch)==1
                ekg=data_org(idx_ch,:);
            else
                ekg=data_org(idx_ch(2),:)-data_org(idx_ch(1),:);
            end
        end
        data=[eeg_;ekg];
        if round(double(Fs_org))~=Fs
            data=resample(data',Fs,Fs_org)';
        end
    end
end
